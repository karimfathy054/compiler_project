#include "include/RulesReader.h"

using namespace std;

void RulesReader::process_definitions() {
...
void RulesReader::process_rules() {
    for (auto &p : rules)
    {
        string &name = p.first;
        string &def = p.second;
...
int RulesReader::precedence(char c) {
    if (c == '|')
    {
        return 1;
    }
...
string RulesReader::convert_to_postfix(string rule) {
    // precedence: | < concat < +,*
    stack<char> s;
    string postfix = "";
    bool concat = false;
...
bool RulesReader::is_input(char c) {
    return not (c == '|' || c == '*' || c == '+' || c == '(' || c == ')' || c == CONCAT);
}

void RulesReader::generate_all_rules() {
...
RulesReader::RulesReader(string rulesFilePath) {
...
vector<pair<string, string>> RulesReader::get_all_rules()
{
    return all_rules;
}

unordered_set<char> RulesReader::get_possible_inputs() {
